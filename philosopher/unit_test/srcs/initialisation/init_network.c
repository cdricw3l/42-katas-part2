...
/*   Updated: 2025/05/18 18:10:43 by cbouhadr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_network	*build_network(t_mutex_data *mutex_data, t_philo **philos,
	int *params, long long ***time_board)
{
	t_network	*network;
...
t_network	*create_network(int *params)
{
	...
	time_board = get_time_multi_board(params[P], TS_SIZE_ARR);
	if (!time_board)
		return (NULL);
