...
/*   Updated: 2025/06/12 12:23:28 by cw3l             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    *thread_philo_cycle(void *p)
{
    ...
    while (state == OFF)
    {
        //safe_print(get_current_time()  - start , philo, WAITING);
        //ft_temporisation(10, 0);
        state = get_state(philo, 0);
        // if(state == ON)
        //     safe_print(get_current_time()  - start , philo, LUNCHED);
    ...
    while (get_state(philo, 0) == ON && philo->pametres[CYCLE] > i)
    {
        ...
        // if(philo->pametres[ID] == 0)
        // {
        //     if(get_cycle_counter(philo) == 3)
        //     {
        //         safe_print(philo->start, philo, DEATH_2);
        //         change_state(philo, 1, OFF);
        //         return(NULL);
        //     }
        // }
