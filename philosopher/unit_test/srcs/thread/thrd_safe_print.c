...
/*   Updated: 2025/06/12 12:06:15 by cw3l             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char *get_action(int action)
{
    ...
    if(action == THINKING)
        return("is thinking");
    if(action == END_SIMU)
        return( "Philo OFF, un philo est mort, termine sont repas et quitte la table");
    if(action == DEATH_2)
        return( "Le philo est mort");
    if(action == WAITING)
        return( "waiting to be launch");
    if(action == LUNCHED)
        return( "Thread is lunched");
    if(action == MOURU)
        return( "Le philo est mort");
    return(NULL);
}
